...
- [ ] **Phase 8M：** 修改”每次看几张”（值改变时）→ 下次点击”查看卡片 / 继续查看”即按新数量重新创建一轮，不继续旧 session
- [ ] **Phase 8M：** 有未完成 session + **未**修改设置 → 仍继续原 session，不重建
- [ ] **Phase 8M：** 设置页修改后只显示”已更新”toast，不出现”下次新开始时生效”等额外说明文案
- [ ] 设置页无”每日目标”等任务化文案
- [ ] 设置页无”下次新开始时生效”文案（Phase 8M 已删除）
