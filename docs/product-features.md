...
- **每次看几张**：picker 选择器，可选值为 `3 / 5 / 10 / 15` 张，默认 5 张。语义从"每天必须完成多少张"改成"每次点看一看 / 继续看时，一组看几张"。新建 session 时使用最新设置；有未完成 session 时点击"继续查看"不重建 session，不受新设置影响。
...
- 新建 session 才使用最新设置；已有未完成 session 时点击"继续查看"不重建。
- 当前卡片不足时实际 session 返回更少卡片，不算 bug。
