...
- **已删除内容**：
  ...
  - 不再有"今日复习情况页"入口（`today_review_status` 已在 Phase 8K 删除）。
...
前端在 blur 后和保存前执行同一套低风险 `normalizeEnglishText`（10 步）；输入过程中不回写英文输入框：
...
- 新增卡片时，只要英文内容变化，始终根据内容自动识别类别：
  - `hello` → 单词
  - `hello world` / `break a leg` → 短语
  - `I am happy.` / `How are you?` → 句子
- 用户手动选择类别后，如果英文内容继续变化，仍会重新按内容自动识别（`hasUserChangedCategory` 不再永久阻止自动识别）。
- 编辑已有卡片时不自动改类别（编辑模式暂保持现有策略）。
- `U.S.`、`e.g.`、`Dr.` 等缩写不会因末尾句点误判为句子。

#### 英文分析触发时机

- Add 页在用户输入后进行 **本地校验 → 自动类别识别**，并触发英文分析请求（500ms debounce）。
- **输入以空白字符结尾时跳过自动分析**，等待用户继续输入、blur 或保存。
- blur 后做一次低风险规范化回写，然后触发分析。
...
用户入口文案：首页普通状态为"查看卡片"，有进行中 session 时为"继续查看"。系统帮用户挑一组值得看的卡片，不强调每日目标打卡。底层仍是复习 session 和反馈逻辑；Phase 8L 后，daily_suggested 新建 session 会在卡片库足够时尽量按“每次看几张”凑满。
...
- **网络异常时的反馈处理（Phase 8J 后）**：
  - 前台反馈失败时弹 toast"网络不可用，请检查当前网络"，页面停留在当前卡片。
  - **不进入下一张**，**不产生 ReviewLog**，**不进入今天看过 / 历史记录**，**不改变卡片复习状态**。
  - 失败 action 会被立即移除出本地队列，**不会**在恢复网络后自动补交。
  - 恢复网络后用户必须**重新点击反馈**，才算真正提交成功。
  - 断网期间用户改点另一档反馈（如先点"记得"失败，再点"没想起"），断网下点击仍失败、不产生记录；最终结果以"恢复网络后那次成功点击"为准。
- **同一 session item 的本地 feedback 去重**：
  - 同一个 `session_item_id` 的本地 pending feedback action 只保留最新一次：`submitReview` 入队前会按 `session_item_id` 移除旧 feedback action，避免旧失败 action 先 flush 把用户最新意图覆盖成 ignored。
  - 去重只按 `session_item_id`，不按 `card_id`，避免误删同卡不同 session item / 回炉 item。
  - 不同 session item / 不同卡片各自独立，互不影响。
- **完成后跳转**：所有卡片反馈完成后自动跳转 `today_reviewed`（今天看过页），若跳转失败则展示页内完成面板。
...
- **每次看几张**：picker 选择器，可选值为 `3 / 5 / 10 / 15` 张，默认 5 张。语义从"每天必须完成多少张"改成"新建一轮查看时的目标数量"。新建 session 时使用最新设置；卡片库可用卡片足够时尽量凑满。修改后无论是否有未完成 session，下次点击"查看卡片 / 继续查看"均按新数量重新创建一轮（Phase 8M）。若未修改设置，有未完成 session 时仍继续原 session 不重建。
- **修改即触发下次重开**：修改"每次看几张"后设置页只显示"已更新"toast，不新增额外说明文案；下次点击"查看卡片"时自动按新数量重开 session。已提交过的反馈记录保留，不回滚。回炉机制不变。
...
- **入口**：设置页 → 关于。
- **产品介绍**：标题"英语知识卡片本"+ 副标题 + 简要说明小程序用途（记录 + 查看卡片轻量回顾）。
- **怎么使用**：4 步说明（添加卡片 / 查看卡片 / 反馈记忆 / 回看记录）。
- **联系开发者**：点击次级按钮弹出开发者邮箱弹窗，可一键复制邮箱（`wx.setClipboardData`），不在页面常驻展示完整邮箱。
- 页面样式与首页浅绿轻量风格一致（白底卡片 + 浅绿背景）。
...
- 可选值：3 / 5 / 10 / 15 张，默认 5 张。非法值 fallback 到 5。
- 底层字段名仍为 `dailyGoal`，语义为”每次点看一看 / 继续看时一组看几张”，而不是每日强制目标。
- **Phase 8M（修改即下次重开）**：修改”每次看几张”后，下次点击”查看卡片 / 继续查看”时，前端检测到批量大小变更标记（`batchSizeChangedNeedsRestart`），忽略当前未完成 session，使用最新数量强制重开新 session（传 `restart: true`）。成功创建后清除标记；若创建失败，标记保留，方便用户重试。已提交的反馈记录保留，不回滚。
- 未修改设置时，有未完成 session 时仍继续原 session，不受 Phase 8M 影响。
- “每次看几张”是新建一轮查看时的目标数量；卡片库可用卡片总数足够时，后端会尽量凑满（Phase 8L 不变）。
- 当前卡片库可用卡片总数不足时，实际 session 返回更少卡片，不算 bug。
- 同一 session 初始卡片不重复；回炉 item 仍由反馈后的原有回炉算法追加。
- 同一张卡当天重复查看不重复增加“今天看过 N 张”的去重计数；ReviewLog 仍记录每次真实反馈。

### 复习会话 fallback 链

- `daily_suggested`（主）→ `new_only`（备）→ `free_review`（终极兜底）。
- `daily_suggested`：新建 session 按目标数量尽量凑满；优先未看过的新卡，其次到期 / 熟悉中 / 需要巩固，再到有点忘 / 最近负反馈；已记得 / mastered / 今天已看过卡仅作为补位，且排在高优先级候选之后。
...
- **Phase 8J 后**：失败 action 会立即移除出本地队列，**不会**自动后台重试。用户必须在恢复网络后**重新点击**反馈，才会真正提交。失败一次的反馈不产生 ReviewLog、不进入今天看过、不改变卡片复习状态。

### UI 文案

- 尽量避免 "local-only" "pending" "sync queue" 等技术词汇。
- 典型提示：
  - 离线新增卡片：底部"网络恢复后会更新学习记录"（add_queue 同步，不影响复习反馈）。
  - 复习反馈失败：toast "网络不可用，请检查当前网络"。
  - 复习页加载失败：标题"卡片加载失败" + 副文案"网络不可用，请检查当前网络"或"暂时无法加载卡片，请稍后重试"。
...
**Phase 8L / 8K 相关验收（历史阶段，仅供历史参考；当前以 Phase 8K-remove-today-review-status-page 为准）**

> 以下 7 条是 Phase 8L 阶段对"今日复习情况页（today_review_status）"链路的验收要求。该页已在 Phase 8K (`34c6660`) 整页删除，复习完成跳转目标改为 today_reviewed。这些条目不再适用，仅作为历史阶段记录保留。

1. ~~**首页 0/5 状态点击今日目标** → 进入 today_review_status not_started 引导页~~（已下线）
2. ~~**首页 >0 已复习状态点击今日目标** → 直接进入 today_reviewed~~（首页已不再展示今日目标入口；今天看过入口仍存在）
3. ~~**首页 0 状态时** → 右侧箭头 `›` 仍显示~~（今日目标入口已下线）
4. **today_reviewed 右上角** → 显示"历史记录 ›"（弱链接），点击进入历史记录页；不再显示"共 X 张卡片"
5. **today_reviewed 结果筛选** → 全部 / 有点忘了 / 记得 切换；筛选为空时友好空态
6. **复习完成后** → redirect 到 `today_reviewed`（不再是 today_review_status）
7. ~~**today_review_status not_started 页面**~~（已下线）
