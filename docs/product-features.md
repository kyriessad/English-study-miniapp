...
- **今日目标板块**：顶部状态卡片展示 `今日目标` 标签、完成数（`min(今日真实完成数, dailyGoal) / dailyGoal`）及进度条；右侧箭头 `›` 始终显示，hover 态始终生效。点击行为为双路由：
  - `actualCompletedToday == 0`（今日尚未复习任何卡片）→ 进入 `today_review_status`（今日复习情况页）的 `not_started` 引导态；
  - `actualCompletedToday > 0`（今日已复习卡片数 > 0）→ 进入 `today_reviewed`（今日复习内容页）。
  - 判断依据为 `actualCompletedToday`（= `goal_progress.completed_unique_today`，当天 distinct card_id 计数），与 dailyGoal 是否完成无关。
...
- **顶部进度文案**：显示"当前进度 X / Y"；卡片内不重复显示进度数字（已删除卡片右上角的 X/Y）。
- **卡片左上角类型标签**：显示"单词 / 短语 / 句子"，不受进度显示改动影响。
...
该页面承担两个角色：**今日 0 复习时的引导页**，以及**复习完成后的结果落地页**（后续可重构为今日复盘页）。
...
- **not_started 状态（0 复习引导页）**：
  - 页面顶部标题行右侧显示弱链接"历史复习内容 ›"（点击进入历史页）；
  - 主体：hero 图标 `●`、标题"今日完成 0 / N"、副文案"还没开始，今天先复习一点"；
  - 主按钮"开始复习"；
  - 底部"每日目标 N 张 调整 ›"（所有状态共用）；
  - **不显示**"暂无复习内容"卡片入口；
  - **不显示**大号"查看历史复习内容"按钮。
- **进入今日复习内容页**：非 0 状态下点击"查看今日复习内容"或"今日已复习内容"跳转 `today_reviewed`。
- **进入历史复习页**：各状态下均有历史入口跳转 `history_reviewed`。
- **每日目标入口**：底部显示"每日目标 N 张"，点击"调整"跳转设置页。
- **复习完成 redirect**：复习结束后由 review.js 自动跳转到本页（所有状态均适用）。
...
- **页面标题区**：主标题"今日复习内容"；右上角弱链接"历史复习内容 ›"（字号 24rpx、颜色 muted 灰绿），点击进入历史复习页；不再显示"共 X 张卡片"。
- **结果筛选**：顶部 3 个筛选丸子，每个显示对应数量：
  - 全部（所有卡片）
  - 待加强（forgot + shaky）
  - 已掌握（got_it + fluent）
...
- `actualCompletedToday == 0` → 点击进入 `today_review_status` 的 `not_started` 引导态。
- `actualCompletedToday > 0` → 点击进入 `today_reviewed`（今日复习内容页）。
- 右侧箭头 `›` 始终显示；hover 态始终生效。
- 此为导航规则，不影响 dailyGoal / goal_progress 计算。
...
| 暂不删除 today_review_status 页面    | 保留，当前用于 0 状态引导和复习完成落地                                    |
| 暂不重构 today_review_status 非 0 状态 | in_progress / all_done / overachieved 等状态暂未复盘化重构              |
| 暂不把首页做成完整复盘页             | 首页保持"轻量进度 + 快速入口"形态                                        |
