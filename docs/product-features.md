...
- **每次看几张**：picker 选择器，可选值为 `3 / 5 / 10 / 15` 张，默认 5 张。语义从"每天必须完成多少张"改成"新建一轮查看时的目标数量"。新建 session 时使用最新设置；卡片库可用卡片足够时尽量凑满；有未完成 session 时点击"继续查看"不重建、不扩容，不受新设置影响。
...
- 新建 session 才使用最新设置；已有未完成 session 时点击"继续查看"不重建、不扩容，也不受当前设置变化影响。
- “每次看几张”是新建一轮查看时的目标数量；卡片库可用卡片总数足够时，后端会尽量凑满。
