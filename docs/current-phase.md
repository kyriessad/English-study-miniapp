...
**类型：** release/deployment + frontend config + docs
**状态：** 已完成 / 发布前真机完整验收进行中 (2026-05-31)
**前端提交：** `70a0fc` switch backend base url to production
**生产后端地址：** `https://api.qingyacard.com`

### 本阶段部署语义

- 小程序正式环境默认访问生产 HTTPS 后端：`https://api.qingyacard.com`。
- 生产后端已部署在腾讯云轻量应用服务器，公网 IP 为 `49.232.134.229`。
- 生产后端由 Nginx 反向代理到 FastAPI：外部请求 → Nginx → `127.0.0.1:8001`。
- 生产后端由 systemd 托管，不应手动长期运行 `uvicorn`。
- 微信公众平台 request 合法域名已配置为 `https://api.qingyacard.com`。
- 前端 `utils/apiClient.js` 默认 `BACKEND_BASE_URL` 已切换为 `https://api.qingyacard.com`。
- `utils/localBackendConfig.js` 仍只是本地调试覆盖文件，不应提交；若该文件存在并设置 `BACKEND_BASE_URL`，会覆盖 `apiClient.js` 默认地址。
- 真机移动数据扫码已通过，说明小程序可访问生产 HTTPS 后端。
- 旧缓存曾导致 `401 Unauthorized`，清除微信开发者工具缓存后恢复正常；这不是服务器、HTTPS、Nginx 或数据库问题。
...
- 后端部署目录：`/opt/english-backend`。
- 因国内服务器访问 GitHub 不稳定，后端部署链路改为：
  - 本地后端仓库 → GitHub / Gitee
  - 腾讯云服务器 → 从 Gitee clone / pull
- Python 虚拟环境已创建：`/opt/english-backend/.venv`。
- 后端依赖已安装：`pip install -r requirements.txt`。

#### 3. PostgreSQL 生产数据库

- PostgreSQL 用户：`english_user`。
- PostgreSQL 数据库：`english_study`。
- 生产 `.env` 使用 `DATABASE_URL=postgresql://english_user:<password>@localhost:5432/english_study` 连接 PostgreSQL。
- 本地开发仍可使用 SQLite `english_analyzer.db`。
- 服务器生产环境使用 PostgreSQL。
- Alembic 有多个 head，因此生产迁移使用：
  - `alembic upgrade heads`
- 已确认迁移上下文为 `PostgresqlImpl`。
- 注意：该步骤只创建 / 升级 PostgreSQL 表结构，不迁移本地 SQLite 数据。
- 当前生产库可作为干净新库使用；如未来需要迁移本地卡片数据，应单独设计数据导入阶段。

#### 4. systemd 后端常驻运行

- 已配置 `english-backend.service`。
- 已确认 `systemctl status english-backend` 为 `active (running)`。
- 已确认 `curl http://127.0.0.1:8001/health` 返回 `{"status":"ok"}`。
- 后端由 systemd 托管，服务器重启后自动启动。
- 部署过程中曾遇到手动 `uvicorn` 占用 `8001` 端口，已通过停止旧进程解决。
- 后续运维命令：
...
- 已配置 Nginx 反向代理：
  - 外部请求 → Nginx → `127.0.0.1:8001`
- 域名已购买并启用：`qingyacard.com`。
- 后端 API 子域名：`api.qingyacard.com`。
- DNS 解析已配置：
  - A 记录：`api` → `49.232.134.229`
- 已用 Certbot 申请并部署 HTTPS 证书：
  - `sudo certbot --nginx -d api.qingyacard.com`
- 腾讯云轻量服务器防火墙已放行：
  - 80
  - 443
- 已验证：
  - `https://api.qingyacard.com/health` 返回 `{"status":"ok"}`。
...
- 已修改 `utils/apiClient.js`：
  - 默认 `BACKEND_BASE_URL` 已从本地地址切换为 `https://api.qingyacard.com`
- 已提交：
  - `70a0fc` switch backend base url to production
- `utils/localBackendConfig.js` 仍是本地调试覆盖文件，不应提交。
- 真机移动数据扫码已通过，说明小程序可以访问生产 HTTPS 后端。
...
3. 审核前检查：
   ...
   - 页面完整性
   - 生产环境无本地地址
   - 不提交 `utils/localBackendConfig.js`
   - 不泄露 `.env`、数据库密码、JWT_SECRET_KEY、微信 Secret、HUNYUAN_API_KEY
...
- 不写 WECHAT_SECRET。
- 不写 HUNYUAN_API_KEY。
- 不把公网 IP 作为小程序正式后端地址；公网 IP 只作为部署记录出现。
- 正式后端地址统一写 `https://api.qingyacard.com`。

## Previous Phase: Phase 8M-review-batch-size-change-takes-effect-next-view
...
| -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------------------------------ |
| Release-P0-production-backend-cutover                    | Release/deployment + frontend config: 腾讯云后端公网 HTTPS、PostgreSQL、systemd、Nginx、域名 DNS、微信 request 合法域名、前端默认生产地址切换、真机移动数据访问通过                                                                                                                                                                                                   | deployed       | `70a0fc`                           |
