...
Page({
  ...
  async start() {
    ...
    if (this.data.activeSessionId) {
      wx.navigateTo({
        url: '/pages/demo/review-session?session_id=' + this.data.activeSessionId + '&size=' + this.data.size
      });
      return;
    }
      ...
      if (!session.sessionId || (!session.items.length && !session.currentItem)) {
        wx.showModal({
          ...
          success: (result) => {
            ...
            else wx.navigateTo({ url: '/pages/demo/discover' });
          }
        });
        return;
      }
      wx.navigateTo({
        url: '/pages/demo/review-session?session_id=' + session.sessionId + '&size=' + this.data.size
      });
    } catch (error) {
  ...
  openLibrary() { wx.switchTab({ url: '/pages/demo/library' }); },
  openDiscover() { wx.navigateTo({ url: '/pages/demo/discover' }); }
});
